...
Future<void> showImportDialog(
  ...
  await showDialog<void>(
    context: context,
    builder: (_) =>
        _ImportDialog(tasks: tasks, controller: controller, language: language),
  );
  controller.dispose();
}
...
class _ImportDialogState extends State<_ImportDialog> {
        ...
        child: Column(
          ...
          children: [
            Align(
              ...
              child: Text(
                tr(widget.language, 'JSON 示例（只读）', 'JSON example (read-only):'),
              ),
            ),
            ...
            OutlinedButton.icon(
              ...
              label: Text(
                tr(widget.language, '选择 JSON 文件', 'Choose JSON file'),
              ),
            ),
            if (_status != null) ...[
  ...
  Future<void> _pickFile() async {
    setState(() {
      _busy = true;
      _status = tr(widget.language, '正在解析文件...', 'Parsing file...');
    });
    try {
      ...
      if (result == null || result.files.single.path == null) {
        setState(() {
          _status = tr(widget.language, '已取消导入', 'Import cancelled');
          _busy = false;
        });
        return;
      }
      ...
      if (parsed is! List) {
        setState(() {
          _status = tr(
            widget.language,
            '格式错误：根节点必须是数组。',
            'Format error: root node must be an array.',
          );
          _busy = false;
        });
        return;
      ...
      for (var i = 0; i < parsed.length; i++) {
        ...
        final recurrenceType = _parseRecurrence(
          node['recurrenceType'] as String?,
        );
        ...
        if (recurrenceType == RecurrenceType.none) {
          ...
          if (deadline == null) {
            issues.add(
              widget.language == AppLanguage.zh
                  ? '第 ${i + 1} 行的 deadline 无法解析'
                  : 'Row ${i + 1} has invalid deadline',
            );
            continue;
          }
        } else {
          if (recurrenceValue == null) {
            issues.add(
              widget.language == AppLanguage.zh
                  ? '第 ${i + 1} 行缺少 recurrenceValue'
                  : 'Row ${i + 1} missing recurrenceValue',
            );
            continue;
          }
      ...
      setState(() {
        _status = widget.language == AppLanguage.zh
            ? '成功导入 ${buffer.length} 项任务${issues.isEmpty ? '' : '；附带提示：${issues.join('；')}'}'
