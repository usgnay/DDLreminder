...
class _AddTaskDialogState extends State<_AddTaskDialog> {
    ...
    return Column(
      ...
      children: [
        ...
        SwitchListTile.adaptive(
          ...
          title: Text(tr(language, '具体到时间', 'Specific time')),
          subtitle: Text(
            tr(
              language,
              '开启后在任务标题下方显示具体时间',
          ...
          Row(
            children: [
              Expanded(
                child: Text(
                  _deadline == null
                      ? tr(language, '请先选择日期', 'Pick a date first')
                      : timeFormatter.format(_deadline!),
                ),
              ),
              TextButton(
                onPressed: _deadline == null ? null : _pickTime,
                child: Text(tr(language, '选择时间', 'Pick time')),
              ),
            ],
          ),
      ],
    ...
    return Column(
      ...
      children: [
        DropdownButtonFormField<RecurrenceType>(
          ...
          decoration: InputDecoration(
            labelText: tr(language, '周期类型', 'Recurrence type'),
          ),
          items: [
            DropdownMenuItem(
              value: RecurrenceType.weekly,
              child: Text(tr(language, '每周', 'Weekly')),
            ),
            DropdownMenuItem(
              value: RecurrenceType.monthly,
              child: Text(tr(language, '每月', 'Monthly')),
            ),
          ],
          onChanged: (value) {
            if (value == null) {
  ...
  Widget _buildWeeklySelector(AppLanguage language) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(tr(language, '星期几', 'Day of week')),
  ...
  Widget _buildMonthlySelector(AppLanguage language) {
    ...
    return DropdownButtonFormField<int>(
      ...
      decoration: InputDecoration(
        labelText: tr(language, '每月日期', 'Day of month'),
      ),
      items: items
          .map(
            (day) => DropdownMenuItem(
              value: day,
              child: Text(
                language == AppLanguage.zh ? '每月 $day 日' : 'Day $day',
              ),
            ),
          )
    ...
    return ListTile(
      ...
      subtitle: Slider(
        ...
        label: language == AppLanguage.zh
            ? '$_recurrenceReminderDays 天'
            : '$_recurrenceReminderDays day(s)',
        onChanged: (value) =>
            setState(() => _recurrenceReminderDays = value.round()),
      ),
    );
  }
  ...
  Future<void> _pickDate() async {
    ...
    final previous = _deadline;
    setState(() {
      _deadline = DateTime(
        picked.year,
        picked.month,
        picked.day,
        _hasSpecificTime ? (previous?.hour ?? 0) : 0,
        _hasSpecificTime ? (previous?.minute ?? 0) : 0,
      );
    });
    if (_hasSpecificTime) {
      await _pickTime();
  ...
  void _submit() {
    ...
    final task = _recurringEnabled
        ? Task.recurring(
            ...
            recurrenceValue: _recurrenceType == RecurrenceType.weekly
                ? _weeklyDay
                : _monthlyDay,
